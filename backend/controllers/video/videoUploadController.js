    ...
    (async () => {
      try {
        ...
        if (video.isSubscriberOnly) {
          ...
          // Embedding is now generated by GeminiSummarizationStep in the pipeline (rate-limited, 384-dim)
          await cloudflareR2Service.uploadFileToR2(tempFilePath, rawVideoKey, tempMimeType);
...
export const registerUpload = async (req, res) => {
  ...
  try {
    ...
    // Non-blocking: Add to queue (embedding generated by GeminiSummarizationStep in pipeline)
    (async () => {
       try {
         await queueService.addVideoJob({
            videoId: video._id,
